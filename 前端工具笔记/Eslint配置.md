...
module.exports = {
    ...
    "rules": {
        "no-unused-vars": "off", // 可以出现未使用的变量
        "semi": ["error", "never"], // 不可以使用分号
        "no-use-before-define": ["error", { "functions": true, "classes": false }], // 禁止在变量为声明前使用，但函数和class除外
        "comma-spacing": ["error", { "before": false, "after": true }], // 要求在逗号后加一个空格
        "func-call-spacing": ["error", "never"], // 函数调用的后面不加空格
        "space-before-function-paren": ["error", "never"], // 函数声明的后面不加空格
        "arrow-spacing": ["error", { "before": true, "after": true }], // 强制箭头函数前后都使用空格
        "no-var": "error" // 禁止使用var变量
    }
}
